...
class _OtpTextFieldState extends State<OtpTextField> {
  ...
  void changeFocusToNextNodeWhenValueIsEntered({
    ...
    if (value.isNotEmpty) {
      ...
      if (indexOfTextField + 1 != widget.numberOfFields) {
        ...
        if (_textControllers[indexOfTextField + 1] != null &&
            _textControllers[indexOfTextField + 1].value.text.isNotEmpty) {
          _textControllers[indexOfTextField + 1].selection = TextSelection(
              baseOffset: 0,
              extentOffset: _textControllers[indexOfTextField + 1].text.length);
        }
      } else {
  ...
  void changeFocusToPreviousWhenBackSpaceEntered({
    @required String value,
    @required int indexOfTextField,
  }) {
    //only change focus to the next textField if the value entered has a length greater than one
  ...
  void onSubmit({@required List<String> verificationCode}) {
    if (verificationCode.every((String code) => code != null && code != '')) {
      widget.controller.value =
          TextEditingValue(text: _verificationCode.join());
  ...
  void onCodeChanged({@required String verificationCode}) {
    ...
    widget.onChange(verificationCode);
  }
}
