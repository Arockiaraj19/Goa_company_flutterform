...
Future<Dio> apiClient() async {
  ...
  _dio.interceptors
    ...
    var headerData=deviceInfo.appType=="web"? '{"app_type":"web","device_type": "desktop","browser": "${deviceInfo.browser.toString()}","browser_version": "${deviceInfo.browserVersion.toString()}"}'
        : deviceInfo.appType=="android"? '{"device_model": "${deviceInfo.deviceName}","imei_no": "${deviceInfo.identifier}","app_type": "android","device_type": "mobile"}'
    ...
    if (error.response.statusCode == 410) {
      ...
      print(refreshToken);
      final response1 =
      await Dio().get(baseUrl+refreshTokenEndpoint,options: Options(
