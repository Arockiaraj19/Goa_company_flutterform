...
class EmailSignUpNetwork{
  ...
  Future<ResponseData> verifyEmailForSignup(String email) async {
    ...
    try {
      ...
      var data= _dio.then((value) async {
        response =await value.post(verifyEmailForSignupEndpoint,data: {
          "email":email
          });
        print(response.data);
  ...
  Future<ResponseData> verifyOtpForSignup(String email, otp) async {
    ...
    try {
      ...
      var data= _dio.then((value) async {
        response =await value.post(verifyOtpEndpoint,data: {
          "otp":otp,
          "email":email
        });
        if (response.statusCode == 200) {
  ...
  Future<bool> resendOtpForEmail(String email) async {
    ...
    try {
      final _dio = authClient();
      var data= _dio.then((value) async {
        response =await value.post(resendOtpEndpoint,data: {
          "email":email
        });
        if (response.statusCode == 200) {
          return true;
  ...
  Future<bool> signUpWithEmail(String email, password) async {
    ...
    try {
      final _dio = authClient();
      var data= _dio.then((value) async {
        response =await value.post(signUpEmailEndpoint,data: {
          "email":email,
          "password":password
        });
        print(response.data);
...
class MobileSignUpNetwork{
  ...
  Future<bool> verifyMobileNoForSignup(String mob) async {
    ...
    try {
      ...
      print(mob);
      var data= _dio.then((value) async {
        response =await value.post(verifyMobileNoForSignupEndpoint,data: {
          "mobile_number":"9688399791"
        });
        if (response.statusCode == 200) {
          return true;
        }
  ...
  Future<bool> signUpWithMobile(String mobile) async {
    ...
    try {
      final _dio = authClient();
      var data= _dio.then((value) async {
        response =await value.post(signUpMobileEndpoint,data: {
